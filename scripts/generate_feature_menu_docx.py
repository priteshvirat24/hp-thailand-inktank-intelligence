...
Enterprise Feature, Capability, Data-Cut Menu & Stakeholder Playbooks (Checklist for David Chiu)

Generates an executive Word document (.docx) complete with:
- Executive Cover, Scope & Metadata
- Master Capability & Data-Cut Checklist Table
- Forensic Data Reconciliation & Lineage Standard
- Tab-by-Tab Deep Dive across all 10 tabs + Global Floating RAG Assistant:
  * Strategic Mission & Executive Context
  * 22 High-Resolution Embedded Screenshots (Scrapling Captures + Ingestion Proofs)
  * Structured Visual Guide & Component Breakdown Under Every Picture
  * Interactive Capabilities & Features Checklist
  * Available Data Cuts & Filter Dimensions (Brand × Month × Channel × SKU × Platform)
  * Technical Caveats & Interpretation Boundaries
  * Dedicated Stakeholder Discussion Playbook & Objection-Handling Script
- Multi-Stakeholder Master Playbooks (MD, Marketing, Commercial, CX Tracks)
"""
...
OUTPUT_FILE = '/Users/priteshhome/InkTank-analysis /HP_Thailand_InkTank_POC_Feature_Menu.docx'

# Color Palette
HEX_PRIMARY = '0F172A'       # Slate 900
HEX_ACCENT = '0284C7'        # Sky 600
HEX_SECONDARY = '334155'     # Slate 700
HEX_BG_LIGHT = 'F8FAFC'      # Slate 50
HEX_BORDER = 'CBD5E1'        # Slate 300
HEX_CARD_BG = 'F1F5F9'       # Slate 100
HEX_ALERT_BG = 'EFF6FF'      # Blue 50
HEX_ALERT_BORDER = '3B82F6'  # Blue 500
HEX_CAVEAT_BG = 'FFFBEB'     # Amber 50
HEX_CAVEAT_BORDER = 'F59E0B' # Amber 500
HEX_PLAYBOOK_BG = 'F0FDF4'   # Green 50
HEX_PLAYBOOK_BORDER = '10B981'# Green 500

COLOR_PRIMARY = RGBColor(15, 23, 42)
COLOR_SECONDARY = RGBColor(51, 65, 85)
...
def set_table_borders(table, color_hex=HEX_BORDER, sz='4'):
...
def add_callout(doc, text_list, title="EXECUTIVE TAKEAWAY", box_type="info"):
    bg_hex = HEX_ALERT_BG if box_type == "info" else (HEX_CAVEAT_BG if box_type == "caveat" else HEX_PLAYBOOK_BG)
    border_hex = HEX_ALERT_BORDER if box_type == "info" else (HEX_CAVEAT_BORDER if box_type == "caveat" else HEX_PLAYBOOK_BORDER)
    ...
    run_title.font.color.rgb = COLOR_ACCENT if box_type == "info" else (RGBColor(180, 83, 9) if box_type == "caveat" else RGBColor(16, 149, 106))
...
def safe_add_image(doc, img_path, caption=None, width=Inches(6.2)):
    if os.path.exists(img_path):
        try:
            ...
            if caption:
                ...
                r2 = cp.add_run(f"\n[Asset: {os.path.basename(img_path)} | 100% visible on Mac via companion HTML (Chrome/Safari) or PDF (Preview)]")
                r2.font.name = 'Calibri'
                r2.font.size = Pt(8)
                r2.font.color.rgb = COLOR_ACCENT
        except Exception as e:
            p = doc.add_paragraph(f"[Image reference: {os.path.basename(img_path)} - {caption}]")
            p.italic = True
    else:
        p = doc.add_paragraph(f"[Image file not found: {os.path.basename(img_path)}]")
        p.italic = True

def add_picture_explanation(doc, figure_title, components_list, interpretation, business_impact):
    """Adds a structured visual walkthrough and explanation below an embedded screenshot."""
    tbl = doc.add_table(rows=1, cols=1)
    tbl.alignment = WD_TABLE_ALIGNMENT.CENTER
    tbl.autofit = False
    tbl.columns[0].width = Inches(6.5)
    ...
    p.paragraph_format.space_before = Pt(2)
    p.paragraph_format.space_after = Pt(4)
    run_title = p.add_run(f"■ SCREENSHOT EXPLANATION & VISUAL WALKTHROUGH: {figure_title.upper()}\n")
    run_title.bold = True
    run_title.font.name = 'Arial'
    run_title.font.size = Pt(9.5)
    ...
    p2.paragraph_format.space_before = Pt(2)
    p2.paragraph_format.space_after = Pt(2)
    r = p2.add_run("What You See in this Live Dashboard Capture:")
    r.bold = True
    r.font.name = 'Calibri'
    r.font.size = Pt(9)
    ...
    r_lbl = p3.add_run("How to Interpret the Displayed Data: ")
    r_lbl.bold = True
    r_lbl.font.name = 'Calibri'
    r_lbl.font.size = Pt(8.5)
    r_lbl.font.color.rgb = COLOR_SECONDARY
    r_val = p3.add_run(interpretation)
    ...
    r_lbl2 = p4.add_run("Executive Demonstration Utility: ")
    r_lbl2.bold = True
    r_lbl2.font.name = 'Calibri'
    r_lbl2.font.size = Pt(8.5)
    r_lbl2.font.color.rgb = COLOR_SECONDARY
    r_val2 = p4.add_run(business_impact)
...
def build_document():
    ...
    r_tag = p_header.add_run("ENTERPRISE INTELLIGENCE SYSTEM · THAILAND INK TANK POC\n")
    r_tag.font.name = 'Arial'
    r_tag.font.size = Pt(9)
    r_tag.font.color.rgb = COLOR_ACCENT
    r_tag.bold = True

    r_title = p_header.add_run("Executive Feature, Capability & Data-Cut Menu\n")
    r_title.font.name = 'Arial'
    r_title.font.size = Pt(22)
    r_title.bold = True
    r_title.font.color.rgb = COLOR_PRIMARY

    r_sub = p_header.add_run("Operational Field Checklist & Demonstration Playbooks for David Chiu\n")
    ...
    set_table_borders(meta_table, HEX_BORDER, sz='4')
    ...
    add_callout(
        doc,
        [
            "This document is an exhaustive, field-ready manual structured tab-by-tab for David Chiu and HP Thailand executive leadership.",
            "All screenshots were captured directly using Scrapling (scrapling.fetchers.DynamicFetcher controlling real Google Chrome in headless mode) at 1440×960 resolution.",
            "Every tab is presented as a complete operational module featuring: (1) Strategic Mission, (2) High-Resolution Screenshots, (3) Visual Component Walkthroughs, (4) Interactive Capabilities, (5) Available Data Cuts, (6) Technical Caveats, and (7) Dedicated Stakeholder Playbooks with Objection-Handling Scripts."
        ],
        title="EXECUTIVE PURPOSE & STRUCTURE STANDARD",
        box_type="info"
    )
    ...
    add_callout(
        doc,
        [
            "1. MAC NATIVE VIEWING NOTICE: Apple's built-in TextEdit application skips modern Office OpenXML DrawingML graphics (<w:drawing>). If opening this .docx file on a Mac without Microsoft Word installed, images will appear blank in TextEdit.",
            "2. 100% VISIBLE SCREENSHOT EDITIONS: To view all 24 high-resolution Scrapling screenshots with full visual fidelity on Mac, please open the companion standalone 'HP_Thailand_InkTank_POC_Feature_Menu.html' in Google Chrome/Safari, or 'HP_Thailand_InkTank_POC_Feature_Menu.pdf' in Apple Preview.",
            "3. CLOUD & OFFICE VIEWING: When uploaded to Google Docs (Google Drive), Word Online (Office 365), or opened on a PC with Microsoft Word, all 24 screenshots and formatting render in full resolution."
        ],
        title="MACOS COMPATIBILITY & SCREENSHOT VISIBILITY NOTICE",
        box_type="warning"
    )
    ...
    r = h1.add_run("1. Master Capability & Data-Cut Menu (The Executive Checklist)")
    r.font.name = 'Arial'
    r.font.size = Pt(15)
    r.bold = True
    r.font.color.rgb = COLOR_PRIMARY
    ...
    p_intro = doc.add_paragraph(
        "Use this master checklist table during internal reviews or partner meetings to verify which analytical capabilities, "
        "filters, and export functions are operational in each tab:"
    )
    p_intro.paragraph_format.space_after = Pt(6)

    checklist_table = doc.add_table(rows=12, cols=5)
    checklist_table.alignment = WD_TABLE_ALIGNMENT.CENTER
    checklist_table.autofit = False
    checklist_table.columns[0].width = Inches(1.1)
    checklist_table.columns[1].width = Inches(1.3)
    checklist_table.columns[2].width = Inches(1.6)
    checklist_table.columns[3].width = Inches(1.4)
    checklist_table.columns[4].width = Inches(1.1)
    set_table_borders(checklist_table, HEX_BORDER, sz='4')

    headers = ["Tab / Workspace", "Core Business Question", "Interactive Capabilities", "Available Data Cuts", "Stakeholder Fit"]
    for col_idx, h_text in enumerate(headers):
        c = checklist_table.cell(0, col_idx)
    ...
    for row_idx, data in enumerate(tab_rows, start=1):
        bg = HEX_BG_LIGHT if row_idx % 2 == 1 else 'FFFFFF'
        for col_idx, text in enumerate(data):
            c = checklist_table.cell(row_idx, col_idx)
    ...
    r = h2.add_run("2. Forensic Data Reconciliation & Lineage Standard")
    r.font.name = 'Arial'
    r.font.size = Pt(15)
    r.bold = True
    r.font.color.rgb = COLOR_PRIMARY
    ...
    p_rec = doc.add_paragraph(
        "Following executive reviews with Sahaj and David Chiu, the POC data pipeline was audited to ensure 100% "
        "mathematical reconciliation across Brand × Month × Channel. Every headline number matches the underlying data:"
    )
    p_rec.paragraph_format.space_after = Pt(4)

    recon_table = doc.add_table(rows=6, cols=5)
    recon_table.alignment = WD_TABLE_ALIGNMENT.CENTER
    recon_table.autofit = False
    recon_table.columns[0].width = Inches(1.3)
    recon_table.columns[1].width = Inches(1.1)
    recon_table.columns[2].width = Inches(1.1)
    recon_table.columns[3].width = Inches(1.4)
    recon_table.columns[4].width = Inches(1.6)
    set_table_borders(recon_table, HEX_BORDER, sz='4')

    r_headers = ["Channel", "Total 90D Records", "Monthly Sample (Aug)", "Deduplication Standard", "Reconciliation Status"]
    for col_idx, h_text in enumerate(r_headers):
        c = recon_table.cell(0, col_idx)
        set_cell_background(c, HEX_PRIMARY)
        set_cell_margins(c, top=80, bottom=80, left=100, right=100)
        p = c.paragraphs[0]
        p.paragraph_format.space_after = Pt(0)
    ...
    for row_idx, data in enumerate(recon_data, start=1):
        bg = HEX_BG_LIGHT if row_idx % 2 == 1 else 'FFFFFF'
        for col_idx, text in enumerate(data):
            c = recon_table.cell(row_idx, col_idx)
            set_cell_background(c, bg)
            set_cell_margins(c, top=60, bottom=60, left=80, right=80)
            p = c.paragraphs[0]
            p.paragraph_format.space_after = Pt(0)
    ...
    add_callout(
        doc,
        [
            "1. AD COUNT AUDIT (DAVID & SAHAJ FEEDBACK): The POC dashboard now provides both metrics: 13 Unique In-Market Creatives (HP: 4, Epson: 3, Canon: 3, Brother: 3) and 52 Monthly Ad Flight Observations (13 creatives × 4 recurring flight cycles). A toggle button lets the user switch between views instantly.",
            "2. CONSUMER STAR RATINGS AUDIT: Star ratings are sourced strictly from verified e-commerce buyers (Shopee Thailand). Pantip.com consumer discussions are preserved as unrated qualitative customer voice to prevent synthetic rating injection.",
            "3. MATHEMATICAL INTEGRITY: 196 HP listings / 1,036 total = 18.9% E-Commerce Shelf Share. In Paid Media, 16 HP flights / 52 total = 30.8% Paid SOV. Every headline card matches the Evidence Lake."
        ],
        title="RECONCILIATION AUDIT INVARIANTS & VERIFICATION PROOF",
        box_type="info"
    )

    # ─────────────────────────────────────────────────────────────────────────
    # SECTION 3: TAB-BY-TAB DEEP DIVE WITH PICTURES & STRUCTURED EXPLANATIONS
    # ─────────────────────────────────────────────────────────────────────────
    h3 = doc.add_paragraph()
    h3.paragraph_format.space_before = Pt(18)
    h3.paragraph_format.space_after = Pt(4)
    r = h3.add_run("3. Tab-by-Tab Deep Dive: Screenshots, Capabilities, Data Cuts & Playbooks")
    r.font.name = 'Arial'
    r.font.size = Pt(15)
    r.bold = True
    r.font.color.rgb = COLOR_PRIMARY

    p_dive = doc.add_paragraph(
        "Each section below reviews one dashboard tab as an autonomous strategic module, containing high-resolution Scrapling "
        "screenshots, structured visual component breakdowns, capabilities, available data cuts, caveats, and tailored stakeholder discussion playbooks."
    )
    p_dive.paragraph_format.space_after = Pt(8)
    ...
    for tab in tabs_detail:
        ...
        r = h.add_run(f"3.{tabs_detail.index(tab)+1} {tab['num']}: {tab['title']}")
        r.font.name = 'Arial'
        r.font.size = Pt(13)
        r.bold = True
        r.font.color.rgb = COLOR_PRIMARY
        ...
        r_lbl = p_obj.add_run("Strategic Mission: ")
        r_lbl.bold = True
        r_lbl.font.name = 'Calibri'
        r_lbl.font.size = Pt(9.5)
        r_val = p_obj.add_run(tab['objective'])
        r_val.font.name = 'Calibri'
        r_val.font.size = Pt(9.5)

        # Embed Images & Structured Visual Walkthroughs
        for img_info in tab['images']:
            safe_add_image(doc, img_info['path'], caption=img_info['caption'], width=Inches(6.2))
            add_picture_explanation(
                doc,
                figure_title=img_info['title'],
                components_list=img_info['components'],
                interpretation=img_info['interpretation'],
                business_impact=img_info['business_impact']
            )
        ...
        r = p_f.add_run("Key Features & Interactive Capabilities:")
        r.bold = True
        r.font.name = 'Arial'
        r.font.size = Pt(9.5)
        ...
        p_dc.paragraph_format.space_before = Pt(4)
        p_dc.paragraph_format.space_after = Pt(2)
        r = p_dc.add_run("Available Data Cuts & Filter Dimensions:")
        r.bold = True
        r.font.name = 'Arial'
        r.font.size = Pt(9.5)
        r.font.color.rgb = COLOR_SECONDARY
        ...
        p_cav.paragraph_format.space_before = Pt(4)
        p_cav.paragraph_format.space_after = Pt(2)
        r = p_cav.add_run("Technical Caveats & Boundary Conditions:")
        r.bold = True
        r.font.name = 'Arial'
        r.font.size = Pt(9.5)
        r.font.color.rgb = COLOR_SECONDARY
        ...
        # Dedicated Tab Stakeholder Playbook
        pb = tab['playbook']
        add_callout(
            doc,
            [
                f"Target Stakeholder Audience: {pb['audience']}",
                f"Core Business Question Addressed: {pb['core_question']}",
                f"Meeting Demonstration Script:\n{pb['script']}",
                f"Objection Handling & Evidence Defense:\n{pb['objections']}"
            ],
            title=f"STAKEHOLDER DISCUSSION PLAYBOOK — {tab['num'].upper()}: {tab['title'].upper()}",
            box_type="playbook"
        )
    ...
    r = h4.add_run("4. Master Cross-Tab Stakeholder Playbooks (4 Demonstration Tracks)")
    r.font.name = 'Arial'
    r.font.size = Pt(15)
    r.bold = True
    r.font.color.rgb = COLOR_PRIMARY
    ...
    p_pb = doc.add_paragraph(
        "End-to-end multi-tab demonstration sequences designed for David Chiu when conducting complete presentations for "
        "different leadership committees:"
    )
    ...
    for pb in playbooks:
        add_callout(
            doc,
            [
                f"Strategic Persona & Focus: {pb['focus']}",
                f"Recommended Demonstration Path: {pb['tabs']}",
                f"Step-by-Step Meeting Script:\n{pb['script']}"
            ],
            title=pb['title'],
            box_type="playbook"
        )
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # SAVE DOCUMENT & REPLICATE ACROSS PATHS
    # ─────────────────────────────────────────────────────────────────────────
    ...
    copy_destinations = [
        '/Users/priteshhome/InkTank-analysis/HP_Thailand_InkTank_POC_Feature_Menu.docx',
        os.path.expanduser('~/Downloads/HP_Thailand_InkTank_POC_Feature_Menu.docx'),
        os.path.expanduser('~/Desktop/HP_Thailand_InkTank_POC_Feature_Menu.docx')
    ]
    for dest in copy_destinations:
        try:
            ...
            if os.path.exists(dest_dir):
                ...
                print(f"Copied replica to: {dest}")
        except Exception as e:
